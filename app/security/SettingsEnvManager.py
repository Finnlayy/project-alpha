...
class SettingsEnvManager:
    ...
    def update_env_variables(self, new_vars: Dict[str, str]) -> bool:
        """Schreibt aktualisierte Umgebungsvariablen in .env und aktualisiert os.environ."""
        try:
            current_vars = self.read_env_file()
            for k, v in new_vars.items():
                if v is not None:
                    clean_v = str(v).strip()
                    current_vars[k.strip()] = clean_v
                    os.environ[k.strip()] = clean_v

            lines = [
                "# Project Alpha - Environment Configuration",
                "# Generated by SettingsEnvManager (v1.7.0)",
                ""
            ]
            ...
            logger.info(".env erfolgreich aktualisiert (%d Variablen geschrieben)", len(current_vars))
            return True
        except Exception as e:
            logger.error("Fehler beim Schreiben der .env-Datei %s: %s", self.env_path, e)
            return False

    def save_kraken_credentials(
        self,
        spot_api_key: Optional[str] = None,
        spot_private_key: Optional[str] = None,
        futures_api_key: Optional[str] = None,
        futures_private_key: Optional[str] = None
    ) -> bool:
        """Speichert Kraken Dual-Key Anmeldedaten sicher in die .env-Datei."""
        updates: Dict[str, str] = {}
        if spot_api_key:
            updates["KRAKEN_SPOT_API_KEY"] = spot_api_key
        if spot_private_key:
            updates["KRAKEN_SPOT_PRIVATE_KEY"] = spot_private_key
        if futures_api_key:
            updates["KRAKEN_FUTURES_API_KEY"] = futures_api_key
        if futures_private_key:
            updates["KRAKEN_FUTURES_PRIVATE_KEY"] = futures_private_key

        return self.update_env_variables(updates)
